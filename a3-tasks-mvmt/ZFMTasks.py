...
def convert_location(big_graph,y,x):
    ...
    for n,d in big_graph.nodes(data=True):
        ...
        if(x_g>maxlong):
            maxlong=x_g
    g_length=great_circle_vec(maxlat,minlong,maxlat,maxlong)#length
    g_width=great_circle_vec(minlat,maxlong,maxlat,maxlong)#width
...
def checkcontact(point,listpoints):
    ...
    for itm in listpoints:
        if(itm[2]!=point[2]):
            if(itm[2]!=ngh):
                dist=great_circle_vec(point[0],point[1],itm[0],itm[1])
                if(dist<ray):
                    count+=1
                    ngh=itm[2]
...
# to calculate the great-circle distance between two points or between vectors of points, using haversine
    dist_or=great_circle_vec(maxlat,minlong,maxlat,maxlong)#length
    dist_ver=great_circle_vec(minlat,maxlong,maxlat,maxlong)#width
    ...
    for u1,v1,dat in list_edges:
        ...
        ##G2.add_node(G3.nodes[u1]['osmid'],nodeu)
        ...
        dista=great_circle_vec(y1,x1,y2,x2)
        ...
        for k in range (0,rel):
            ...
            dist=great_circle_vec(ty,tx,y3,x3)

            du=great_circle_vec(y1,x1,y3,x3)
            dv=great_circle_vec(y2,x2,y3,x3)
        ...
        #if i>1:
            #dist=great_circle_vec(ty,tx,y2,x2)
...
if  name_city!='no' :
    ...
    if(antenna_decision==1):
        ...
        for i in range(0,len(list_group)):
            ...
            if len(list_group[i])!=0:
                ...
                afile.write("%s %s %s\n" % (c,float(G_big.nodes[antenna]['y']), float(G_big.nodes[antenna]['x'])))
                #afile.write("%s %s %s\n" % (c,float(G_big.nodes[antenna]['y']), float(G_big.nodes[antenna]['x'])))
    afile.close()
    if ch=='y':
   ...
   # wfile = open('punti.txt', 'w')
    #for item1 in G_big.nodes():
       # wfile.write(" %s \n" % [G_big.nodes[item1]['x'],G_big.nodes[item1]['y'],G_big.nodes[item1]['group']])
    ...
    for run_num in range(1,11):
        ...
        attackLo_file = open(f'{os.getcwd()}/Inputs/Mobility/differentradius/50/'+str(run_num)+'/AttackLocations.txt','w')
